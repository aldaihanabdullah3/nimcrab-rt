...
// ─── Syscall trampoline ───────────────────────────────────────────────────────
// Naked fn: load SSN into eax, first 4 args already in rcx/rdx/r8/r9,
// args 5+ sit at rsp+0x28 per x64 calling convention.
...
pub unsafe extern "system" fn syscall_trampoline(
    ...
    // a3   → r9
    // a4   → rsp+0x28 (5th arg)
    // But NT syscall: rcx=a1, rdx=a2, r8=a3, r9=a4
    // SSN must be in eax; r10 = rcx before syscall
    asm!(
        "mov r10, rcx",       // r10 = ssn (ignored — we move eax below)
        "mov eax, ecx",       // eax = ssn
        "mov rcx, rdx",       // rcx = a1
        "mov rdx, r8",        // rdx = a2
        "mov r8,  r9",        // r8  = a3
        "mov r9,  [rsp+0x28]",// r9  = a4 (from stack)
        "mov r10, rcx",       // r10 = rcx (required by NT syscall ABI)
        "syscall",
        "ret",
        options(noreturn)
    )
...
pub unsafe fn do_syscall(ssn: u16, a1: usize, a2: usize, a3: usize, a4: usize,
    ...
    // For calls with >4 args we use inline asm to place extra args on stack
    let result: i32;
    asm!(
        "sub rsp, 0x50",
        "mov [rsp+0x28], {a5}",
